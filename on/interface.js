(function () {
	...
	Lampa.SettingsApi.addParam({
		...
		onRender: function (item) {
			setTimeout(function() {
				if(Lampa.Storage.field('interface1')) item.show()&$('.settings-param__name', item).css('color','f3d900')&$('div[data-name="new-interface1"]').insertAfter('div[data-name="interface_size"]');
					else item.hide();
			}, 0);
        }
    });
    ...
    function create() {
      ...
      this.update = function (data) {
        html.find('.new-interface-info__head,.new-interface-info__details').text('___');
    ...
    function startPlugin() {
	  ...
	  if (Lampa.Storage.get('new-interface1', false) == false) return;
      var old_interface = Lampa.InteractionMain;
      var new_interface = component;

      Lampa.InteractionMain = function (object) {
        var use = new_interface;
        if (!(object.source == 'kinovod', object.component == 'category'|| object.source == 'tmdb' || object.source == 'cub')) use = old_interface;
