(function () {
    ...
    function Jaja(object) {
        ...
        this.card = function (str) {
            ...
            var catalogs1 = getCatalogs().filter(function (fp) {
            ...
            if (count == 0) {
                ...
                if (count == 1) {
                    host_img = object.url.match(/(http|https):\/\/(www.)?(\w+(\.)?)+/)[0];
                } else {
        ...
        this.build = function (data) {
            ...
            this.selectGroup = function () {
                ...
                Lampa.Select.show({
                    title: 'Источник',
                    items: getCatalogs().map(function (elem) {
    ...
    function listNavigation() {
        if (Lampa.Storage.get('online_jaja_balanser') == '') {
            Lampa.Storage.set('online_jaja_balanser', getCatalogs()[0].title);
        }
    ...
    function listNavigation2() {
        if (Lampa.Storage.get('online_jaja_balanser') == '') {
            Lampa.Storage.set('online_jaja_balanser', getCatalogs2()[0].title);
        }

        var balanser = Lampa.Storage.get('online_jaja_balanser');

        var catalogs1 = getCatalogs2().filter(function (fp) {
            return fp.title === balanser;
        });

        if (catalogs1.length === 0) {
            catalogs1[0] = getCatalogs2()[0];
            Lampa.Storage.set('online_jaja_balanser', getCatalogs2()[0].title);
        };
    ...
    function listNavigation3() {
        if (Lampa.Storage.get('online_jaja_balanser') == '') {
            Lampa.Storage.set('online_jaja_balanser', getCatalogs3()[0].title);
        }

        var balanser = Lampa.Storage.get('online_jaja_balanser');

        var catalogs1 = getCatalogs3().filter(function (fp) {
            return fp.title === balanser;
        });

        if (catalogs1.length === 0) {
            catalogs1[0] = getCatalogs3()[0];
            Lampa.Storage.set('online_jaja_balanser', getCatalogs3()[0].title);
        };
