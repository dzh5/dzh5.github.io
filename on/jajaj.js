(function () {
    ...
    function jaja(object) {
        ...
        var last;
        var waitload = false; 
        var total_pages_from_parser = 0; // Общее количество страниц с сайта
        ...
        var activity = {
            ...
            page: 1, // Текущая страница, которую ЗАПРАШИВАЕМ. Начинаем с 1.
        };
        ...
        this.create = function () {
            ...
            activity.page = 1; // Всегда начинаем с первой страницы
            total_pages_from_parser = 0; 
            ...
            if (object.type == 'fav') {
                ...
                total_pages_from_parser = favData.total_pages; 
                _this.build(favData); 
                waitload = false;
                this.activity.loader(false);
            } else {
                var url_to_load = object.url;
                // Для Jable, если это первая страница и нет page=, добавляем page=1 и lang=en
                if (url_to_load.includes('jable.tv') && url_to_load.indexOf('page=') === -1) {
                    if (url_to_load.includes('?')) {
                        url_to_load += '&page=1';
                    } else {
                        url_to_load += '?page=1';
                    }
                    if (url_to_load.indexOf('lang=en') === -1) {
                         var q_mark_present = url_to_load.includes('?');
                         var existing_params = q_mark_present ? url_to_load.split('?')[1] : '';
                         ...
                         if(url_to_load.endsWith('page=1') && param_separator === '?') param_separator = '&'; 
                         url_to_load += param_separator + 'lang=en';
                     }
                }

                network["native"](current_cors_to_use + url_to_load, function (str) {
                    var parsedData = _this.card(str); 
                    total_pages_from_parser = parsedData.total_pages;
        ...
        this.next = function () { 
            ...
            if (waitload || (total_pages_from_parser > 0 && activity.page >= total_pages_from_parser)) {
                return; 
            }

            waitload = true;
            // this.activity.loader(true); // Анимацию загрузки для next убрали
            ...
            activity.page++; // Инкрементируем номер страницы для запроса

            var url_to_request = object.url; // Базовый URL 
            url_to_request = url_to_request.replace(/&?page=\d+/, ''); // Удаляем старый page=
            if (url_to_request.endsWith('?')) { 
            ...
            if (object.setup && object.setup.use_referer && object.url) {
                try {
                    headers['Referer'] = object.url; // Используем оригинальный URL как реферер для простоты
                } catch (e) {}
            }
            ...
            network["native"](current_cors_to_use + url_to_request, function (result) {
                ...
                if (parsedData.card.length > 0) {
                    ...
                    activity.page--; // Вернули назад, т.к. страница оказалась пустой
                    total_pages_from_parser = activity.page; 
                    waitload = true; 
                }
                // this.activity.loader(false); 
            }, function (a, c) {
                if (a.status == 404) {
                    // Lampa.Noty.show('Страница ' + activity.page + ' не найдена (404).');
                    activity.page--; 
                    total_pages_from_parser = activity.page; 
                } else {
                    Lampa.Noty.show(network.errorDecode(a, c));
        ...
        this.card = function (str_data_from_network) { // Убрали currentPageNumberForParsing
            var card_list = []; 
            var next_page_url_parsed; // Этот URL нужен для this.next в некоторых стратегиях, но не в этой
            var total_pages_parsed = activity.page; // Инициализируем текущей страницей
            ...
            if (pagination_elements.length > 0) {
                ...
                // Если нет пагинации, но есть карточки, считаем, что это одна страница
                // Если карточек нет, то total_pages_parsed останется activity.page (или 1, если первая загрузка)
                if ($(v_selector, html_content_str).length > 0) {
            ...
            // `page` теперь не используется напрямую для формирования URL в this.next, 
            // но его можно оставить для информации или если парсер может точно определить URL следующей страницы
            return { card: card_list, page: undefined, total_pages: total_pages_parsed };
        };
    ...
    // --- НАЧАЛО ЧАСТИ 2 ---
        ...
        this.append = function (data_to_append, is_append_operation) { 
            ...
            data_to_append.card.forEach(function (element) {
                ...
                var original_img_html = card_img_container.html(); // Сохраняем исходный HTML с <img> (если он там есть)
                ...
                var showImage = function() {
                    ...
                    // Восстанавливаем или создаем img
                    ...
                    } else { // Если img уже есть, просто убедимся что src правильный
                        var existing_img = card_img_container.find('img')[0];
                        if (element.img && existing_img.src !== element.img) existing_img.src = element.img;
                        else if (!element.img) existing_img.onerror();
                    }
                };
                
                showImage(); // Показываем картинку при первой отрисовке карточки
                ...
                card_element.on('hover:focus', function () {
                    ...
                    // Запускаем превью с небольшой задержкой
                    clearTimeout(preview_timeout);
                    preview_timeout = setTimeout(function() {
                        if (card_element.hasClass('focus') && element.preview) { // Проверяем, что элемент все еще в фокусе
                            ...
                            video_preview.on('canplaythrough', function() { // Ждем, пока достаточно данных для плавного воспроизведения
                    ...
                    if (!element.preview && element.img) { // Если нет превью, меняем фон Lampa
                ...
                card_element.on('hover:lost', function() {
                    clearTimeout(preview_timeout); // Отменяем запуск превью, если фокус ушел быстро
                    showImage(); // Восстанавливаем изображение
                });
        ...
        this.build = function (data_for_build) {
            ...
            if (data_for_build && data_for_build.card && data_for_build.card.length) { 
                ...
                scroll_element.off('scroll.jaja_autonext wheel.jaja_autonext'); // Удаляем старые обработчики
                ...
                var scrollHandler = function(event) { 
                    ...
                    scroll_timeout = setTimeout(function() { 
                        ...
                        if (isWheelScrollDown || isRegularScroll) { 
                            ...
                            if (el.scrollHeight - el.scrollTop - el.clientHeight < 250 ) { // Порог для подгрузки
                               _this2.next(); 
                            }
                        }
                    }, 150); 
                ...
                scroll_element.on('wheel.jaja_autonext', scrollHandler); // Используем тот же обработчик для wheel
