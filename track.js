(function () {
    ...
    var connect_host = 'http://5.42.82.10:9118';
    var list_opened = false;
    function log() {
      console.log.apply(console.log, arguments);
    }
    function reguest(data, call) {
      var net = new Lampa.Reguest();
      var url = connect_host + '/ffprobe?media=' + encodeURIComponent(data.url);
      var email = Lampa.Storage.get('account_email');
      if (email) url = Lampa.Utils.addUrlComponent(url, 'account_email=' + encodeURIComponent(email));
      log('Tracks', 'reguest', url);
      net.timeout(1000 * 15);
    ...
    function parseMetainfo(data) {
      ...
      reguest(data.element, function (result) {
        if (list_opened && result && result.streams) {
    ...
    Lampa.Player.listener.follow('start', function (data) {
      if (!(/\.m3u8/.test(data.url) || /\.mpd/.test(data.url) || /\.mp4/.test(data.url))) subscribe(data);
    });
